...
df = pd.read_csv(r'/Users/victorakolo/Bigdata/data.csv')
df.head(), df.shape, df.info(), df.columns, df.isnull().sum()
...
sns.heatmap(df_numeric.corr(), annot=True, linewidths=.5, cmap="YlOrBr")
...
sns.pairplot(df[s_col],hue = 'diagnosis', palette='PuRd')
...
sns.barplot(x= model_acc, y=model_name, palette='crest')
